...
async function initializeApp() {
  try {
    ...
    if (urlParams.get("auth") === "success") {
      showNotification("Successfully connected to Bungie.net!", "success");
...
async function loadInventory() {
  ...
  try {
    ...
    if (response.profileInventory?.data?.items) {
      const vaultArmor = response.profileInventory.data.items.filter((item) =>
        isArmorItem(item)
      );
    ...
    if (response.characterInventories?.data) {
      for (const [characterId, inventory] of Object.entries(
        ...
        const charArmor = inventory.items.filter((item) => isArmorItem(item));
...
function findOptimalBuilds() {
      ...
      for (const chest of armorByType.chest) {
        for (const legs of armorByType.legs) {
          for (const classItem of armorByType.classItem) {
            ...
            if (
              selectedExoticId &&
              !pieces.some((p) => p.itemInstanceId === selectedExoticId)
            ) {
              continue;
            }
  ...
  results.sort(
    (a, b) =>
      ...
      Object.values(a.totals).reduce((sum, val) => sum + val, 0)
  );
