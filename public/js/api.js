...
class D2ApiClient {
    ...
    async getCharacter(characterId, components = '200') {
        return this.request(`/api/character/${characterId}?components=${components}`);
    }
    ...
    async getItemDefinitions(itemHashes) {
        ...
        const uncached = itemHashes.filter(hash => !this.manifestCache.has(`item:${hash}`));
        ...
        const result = {};
        itemHashes.forEach(hash => {
            result[hash] = this.manifestCache.get(`item:${hash}`) || definitions[hash];
        });
        return result;
    }
