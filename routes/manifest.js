...
router.post('/items', async (req, res) => {
    try {
        ...
        const definitions = await getDefinitions('DestinyInventoryItemDefinition', hashes);
...
router.get('/item/:hash', async (req, res) => {
    try {
        ...
        const definition = await getCachedDefinition(
            'item',
            hash,
...
router.get('/stats', async (req, res) => {
    try {
        const stats = await getCachedDefinition(
            'stats',
            'all',
            () => getStatDefinitions()
        );
...
router.get('/mods/armor', async (req, res) => {
    try {
        const mods = await getCachedDefinition(
            'mods',
            'armor',
            () => getAllArmorMods()
        );
...
router.get('/buckets', async (req, res) => {
    try {
        const buckets = await getCachedDefinition(
            'buckets',
            'all',
...
router.get('/classes', async (req, res) => {
    try {
        const classes = await getCachedDefinition(
            'classes',
            'all',
...
router.get('/damageTypes', async (req, res) => {
    try {
        const damageTypes = await getCachedDefinition(
            'damageTypes',
            'all',
...
router.post('/update', async (req, res) => {
    try {
        ...
        res.json({ 
            success: true, 
            updated,
            message: updated ? 'Manifest updated successfully' : 'Manifest already up to date'
        });
...
router.get('/status', async (req, res) => {
    try {
        ...
        const versionFile = path.join(__dirname, '..', 'data', 'manifest', 'version.json');
        let version = null;
